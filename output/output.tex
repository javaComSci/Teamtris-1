...
\vspace*{1\baselineskip}\opt
\noindent
\textbf{Description}: When the owner leaves the backend will send a notification with the new owner. If the new owner is this current player, then we will have to start drawing the owner buttons on the screen so they can start the game. We use the \mintinline[fontsize=\footnotesize]{javascript}{Buttons()} class to aid us in this process. 
...
\vspace*{1\baselineskip}\opt
\noindent
\textbf{Description}: This will be called 60 times a second when \mintinline[fontsize=\footnotesize]{javascript}{gameState == 1}. \mintinline[fontsize=\footnotesize]{javascript}{"sketch.js"} is where it takes care of the routing for this. 
...
\vspace*{1\baselineskip}\opt
\noindent
\textbf{Description}: Will be called when the user clicks on either the "Add bot" or "Remove bot" button. \mintinline[fontsize=\footnotesize]{javascript}{mouseClickedLobby()} is who calls this function. This function will form and send a response based on which button you clicked. \\Add Bot: 
\begin{minted}[fontsize=\footnotesize]{javascript}
...
\vspace*{1\baselineskip}\opt
\noindent
\textbf{Description}: draws the title (\mintinline[fontsize=\footnotesize]{javascript}{"lobby"}) onto the screen for all players in the lobby. 
...
\vspace*{1\baselineskip}\opt
\noindent
\textbf{Description}: This function will handle all key presses by the user. Since we only have one place to press a key, the team box, when \mintinline[fontsize=\footnotesize]{javascript}{this.lobbyGameState} is set to \mintinline[fontsize=\footnotesize]{javascript}{0}, we will append to the teamname. The teamname cannot go above 11 chars or below 0. 
...
\end{minted}
 \mintinline[fontsize=\footnotesize]{javascript}{"mouseClicked.js"} is responsible for routing clicks to the correct screen. 



\section{ScoreScreen}
\opt\textbf{Author}: Steven Dellamore, JavaComSci \\\opt
\noindent
\textbf{Description}: Displays the score screen to the user when the \mintinline[fontsize=\footnotesize]{javascript}{gameState = 3}. \\
...
\vspace*{1\baselineskip}\opt
\noindent
\textbf{Description}: ScoreScreen is going to setup all init values for the screen. 
\begin{minted}[fontsize=\footnotesize]{javascript}
this.player = player;
...
\end{minted}
 and finally, we have to listen for incoming packets from the server: 
\begin{minted}[fontsize=\footnotesize]{javascript}
socket.onmessage = (event) => {
    // Listen for backend packets.
    // Parse them and call the correct functions
...
\textbf{\large{\\Returns}}:\\
\textbf{ScoreScreen }: An object of scorescreen class\\
...
\vspace*{1\baselineskip}\opt
\noindent
\textbf{Description}: Will be called whenever the user clicks and \mintinline[fontsize=\footnotesize]{javascript}{gameState == 3}. Will render a new startScreen if the user clicks on the back button. 



\subsection{keyPressedScore}
\opt
\opt\begin{lstlisting}
keyPressedScore()
\end{lstlisting} 
...
\vspace*{1\baselineskip}\opt
\noindent
\textbf{Description}: This will be called 60 times a second in \mintinline[fontsize=\footnotesize]{javascript}{"sketch.js"}. It will render everything onto the screen. 



\section{Player}
\opt\textbf{Author}: Steven Dellamore, Richard Hansen \\\opt
\noindent
...
\textbf{boolean owner }: \mintinline[fontsize=\footnotesize]{javascript}{true} or \mintinline[fontsize=\footnotesize]{javascript}{false} if they are owner\\\textbf{\large{\\Returns}}:\\
...
\textbf{\large{\\Parameters}}:\\
\textbf{int num }: sets the \mintinline[fontsize=\footnotesize]{javascript}{this.playerNum = num}\\\textbf{\large{\\Returns}}:\\
...
\opt\textbf{Author}: Steven Dellamore, Richard Hansen 
\vspace*{1\baselineskip}\opt
\opt\begin{lstlisting}
mouseClicked()
\end{lstlisting} 
...
\section{SingleBot}
\opt\textbf{Author}: JavaComSci \\\opt
\noindent
...
\textbf{boolean debug }: \mintinline[fontsize=\footnotesize]{javascript}{true} if you want debug statments printed\\\textbf{\large{\\Returns}}:\\
...
\opt\textbf{Author}: Richard Hansen 
\vspace*{1\baselineskip}\opt
\opt\begin{lstlisting}
